...
<head>
    ...
    <!-- TITLE OF SITE -->
    <title>Unity Library (Volunteer Registration)</title>
...
<body>
    <?php
        $volunteerHours = isset($_POST['volunteerHours']) ? htmlspecialchars($_POST['volunteerHours']) : "";

		// Initially, none of user inputs meet requirements
		$volunteerHoursOK = $checkOK = false;
        ...
        if(isset($_POST['volunteer'])){
			 ...
			 // Checking to make sure volunteerHours has a value
             if(!empty($_POST['volunteerHours'])) {
				$volunteerHours = htmlspecialchars($_POST['volunteerHours']);
                $volunteerHoursOK = true;
			} else {
                if($volunteerHours == 0){
                    $volunteerHoursOK = true;
                } else {
                    $message = $message . "<p class=\"error-message\">Please enter amount of volunteer hours completed (0 or more)<br>";
                    $volunteerHoursOK = false;
                }
			} // end volunteerHours check

            // Checking if checkbox is checked
            if (isset($_POST['check'])) {
                $isChecked = isset($_POST['check']) ? 'checked' : '';
                $checkOK = true;
            } else {
                $message =  $message . "<p class=\"error-message\">Please accept volunteer checkbox agreement</p>";
                $checkOK = false;
            } // end checkbox checking

			// Checking to ensure all input fields are valid 
			if($volunteerHoursOK && $checkOK){
				// Store information in session storage to save it
            	session_start();
                $role = "volunteer";
            	$_SESSION['volunteerHours'] = $volunteerHours;
                $_SESSION['role'] = $role;
            	header('Location: volunteerResults.php');
            	exit;
			}
        } // end register post
    ?>
            ...
            <section class="service">
				<div class="container">
					<div class="service-details">
                        <div style="width:400px" class="input-box">
                            <h2 class="user-registration">Volunteer Sign-Up</h2>
					        ...
					        <div style="border-radius:10px; padding: 2px 20px" class="single-contact-box">
                                <div class="form-group">
                                    <input type="text" pattern="^[0-9]+$" style="border-radius:10px; padding: 22px 10px" class="form-control" placeholder="Number of Volunteer Hours Complete (0, 1, etc)*" name="volunteerHours" value="<?php print htmlspecialchars($volunteerHours); ?>">
                                </div>
                                <input type ="checkbox" id="check" name="check" <?= $isChecked ?>>
					    	    <label for="check">I agree to be a library volunteer*</label>
                                <button type="submit" style="margin-top:10px; margin-bottom: -10px; width: 350px;" class="submit-button" value="volunteer" name="volunteer">Complete Volunteer Sign-Up</button>			
                            </div>
