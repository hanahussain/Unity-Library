...
<head>
    ...
    <!-- TITLE OF SITE -->
    <title>Unity Library (Book Review)</title>
...
<body>
    <?php
        ...
        if(isset($_POST['reviewBook'])){
            ...
            // Checking to make sure review has a value
            ...
            header('Location: bookReviewResults.php');
        } // end register post
    ?>
            ...
            <section class="service">
				<div class="container">
					<div class="service-details">
                        <div style="width:470px" class="input-box">
                            <h2 class="user-registration">Review Book</h2>
							    <div>
									<label for="dropdown" style="margin-left:20px">Select book to review:</label>
                                    <div style="display:flex">
									<select id="dropdown" name="book" style="width: 200px; padding-top: 5px; padding-bottom: 5px; padding-left: 20px; border-radius: 10px; margin-left: 20px;" required>
										<option value="" disabled selected>Book Selection*</option>
										<?php
                                            // Loop through the $booksList array and create options
                                            foreach ($booksList as $bookie) {
                                                $value = htmlspecialchars($bookie); // Sanitize the book name if needed
                                                $selected = ($book == $bookie) ? 'selected' : ''; // Check if it's the selected option
                                                print '<option value="' . $value . '" ' . $selected . '>' . $bookie . '</option>';
                                            }
                                        ?>
									</select>
                                    </div>
                                <div>
                                    <label for="review" style="margin-top: 20px; margin-left: 10px; padding-left: 10px">Review:</label>
                                    <textarea id="review" name="review" rows="5" cols="0" style="border-radius:10px; padding-top: 10px; width: 89%; margin-left:20px;" class="form-control" placeholder="Type Review Here*" required><?php print isset($review) ? htmlspecialchars($review) : ''; ?></textarea>
                                </div>
								</div>
                                <button type="submit" style="margin-top: 15px; margin-left: 15px; margin-right: 15px; margin-bottom: -15px; width: 425px;" class="submit-button" value="reviewBook" name="reviewBook">Review Book</button>			
                            </div>
